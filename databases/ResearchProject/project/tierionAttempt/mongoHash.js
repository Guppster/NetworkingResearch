...
if (options.ValidateAll) {
  ...
  Promise
    .all([db0, authToken])
    .then((params) => { // Wait for connection promises
      const db = params[0];
      checkHash(db).then(() => {
        log('Checked all hashes');
        checkHash(db, db.databaseName, options.collection, query, projection).then(() => {
          log('checked specific hashes');
          process.exit(0);
        });
      });
    });
  ...
  Promise
    ...
    .then((params) => { // Wait for connection promises
      ...
      hash.then((h) => {
        log('hash=' + h);
        saveHashDb(db, db.databaseName, options.collection, h, query, projection).then((o) => {
          log('DB insert result=' + JSON.stringify(o.result));
...
function saveHashDb(db, dbName, collection, hash, query, projection) {
  ...
  log('Updating hash for ', filterData);
...
function lookupTxn(transactionId) {
  ...
  const result = new Promise((resolve) => {
    restClient.get(txRest, (data, response) => {
      // parsed response body as js object raw response
      debuglog(response);
      debuglog('*', data, '**');
      resolve(data);
    });
...
function commandLineOptions() {
  const usage = 'Usage: -u MongoURI -c collectionName -q query [-p projection] -U tierionUsername' +
      ' -P tierionPassword [-V] [-D]';
